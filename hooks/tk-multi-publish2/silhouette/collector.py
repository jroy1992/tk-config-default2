...
SILHOUETTE_SESSION_ITEM_TYPES = {
    "silhouette.session": {
        "icon_path": "{self}/hooks/icons/file.png",
        "type_display": "Silhouette Session"
    },
}


class SilhouetteSessionCollector(HookBaseClass):
    """
    ...
    def __init__(self, parent, **kwargs):
        ...
        """
        # call base init
        super(SilhouetteSessionCollector, self).__init__(parent, **kwargs)
    ...
    def settings_schema(self):
        ...
        """
        schema = super(SilhouetteSessionCollector, self).settings_schema
        schema["Item Types"]["default_value"].update(SILHOUETTE_SESSION_ITEM_TYPES)
    ...
    def process_current_session(self, settings, parent_item):
        ...
        session_item = self.collect_current_silhouette_session(settings, parent_item)
    ...
    def collect_current_silhouette_session(self, settings, parent_item):
        ...
        if not active_project or not active_project.path:
            ...
            self.logger.warning(
                "The silhouette project has not been saved.",
    ...
    def _resolve_work_path_template(self, settings, item):
        ...
        if item.type == "silhouette.session" and self.__workfiles_app:
        ...
        return super(SilhouetteSessionCollector, self)._resolve_work_path_template(settings, item)
    ...
    def _resolve_item_fields(self, settings, item):
        ...
        fields = super(SilhouetteSessionCollector, self)._resolve_item_fields(settings, item)
        work_template_name = item.properties.get("work_path_template")

        if work_template_name:
            work_template = self.parent.get_template_by_name(work_template_name)
            fields.update(work_template.get_fields(item.properties.path))
